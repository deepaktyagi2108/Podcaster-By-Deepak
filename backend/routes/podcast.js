...
//     const frontImage = req.files?.["frontImage"]?.[0]?.path; // old
//     const audioFile = req.files?.["audioFile"]?.[0]?.path;   // old
...
//     // Remove duplicate file checks (Cloudinary ensures uniqueness by public_id)
...
// ➤ Get podcasts by a specific user
router.get("/get-user-podcasts", authMiddleware, async (req, res) => {
  try {
    const { user } = req;
    ...
    console.error("User Podcast Error:", error);
    return res.status(500).json({ message: "Internal server error" });
  }
});
...
router.delete("/delete-podcasts/:id", authMiddleware, async (req, res) => {
  try {
    ...
    // Delete audio and image from local storage (if stored locally)
    if (podcast.audioFile && fs.existsSync(podcast.audioFile)) {
      fs.unlinkSync(podcast.audioFile);
    }
    if (podcast.frontImage && fs.existsSync(podcast.frontImage)) {
    ...
    return res.status(500).json({ message: "Unable to delete podcast" });
  }
});
...
router.get("/search", async (req, res) => {
  try {
    ...
    console.error("Search Error:", error);
    return res.status(500).json({ message: "Failed to search podcasts" });
  }
});

...
const DescriptionPage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const res = await axios.get(
        `http://localhost:1000/api/v1/podcast/get-podcast/${id}`,
  ...
  const handlePlay = () => {
    ...
    dispatch(playerActions.changeSong(`http://localhost:1000/${Podcasts.audioFile}`));
    dispatch(playerActions.changeImage(`http://localhost:1000/${Podcasts.frontImage}`));
